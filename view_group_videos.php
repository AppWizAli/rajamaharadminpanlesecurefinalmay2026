...
$sql = "SELECT gv.group_id, e.id as video_id, e.video_path, e.episode_number, s.season_number, d.name as drama_name
        FROM group_videos gv 
        JOIN episode e ON gv.video_id = e.id 
    ...
    <div class="dashboard-header mb-4 p-3 rounded shadow-sm bg-white">
        <div class="d-flex flex-wrap align-items-center justify-content-between">
            <div class="page-title">
                <h4 class="mb-0">Group Videos</h4>
                <small>Manage and organize your group videos below</small>
            </div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 p-0">
                    <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Group Videos</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <ul class="row">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) { 
                ...
                echo '            </p>';
                // Delete Video Button with Icon and Shortened Text
                echo '            <a href="delete_group_video.php?group_id=' . $row['group_id'] . '&video_id=' . $row['video_id'] . '" onclick="return confirm(\'Are you sure you want to delete this video from the group?\')" class="btn btn-blue">';
                echo '                <i class="fas fa-trash-alt"></i> Video';
                echo '            </a>';
                echo '        </div>';
                echo '    </div>';
                echo '</li>';
            ...
            echo "No videos assigned to this group.";
        }
        $stmt->close();
        $conn->close();
        ?>          
