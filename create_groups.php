...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_group'])) {
    ...
    $group_comment = isset($_POST['group_comment']) ? $_POST['group_comment'] : ''; // Added comment variable

    if (!empty($group_name) && !empty($group_comment)) {
        $sql = "INSERT INTO `groups` (group_name, group_comment) VALUES (?, ?)"; // Updated SQL to include comment
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
            die("Error preparing SQL: " . $conn->error);
        }
        $stmt->bind_param("ss", $group_name, $group_comment); // Updated to bind comment
        if ($stmt->execute()) {
   ...
   <div class="card mb-3">
    ...
    <div class="card-body">
        <form method="post" action="">
            ...
            <div class="form-group">
                <label for="end_date">Subscription End Date (Optional):</label>
                <input type="date" id="end_date" name="end_date" class="form-control">
                <small class="text-muted">If left empty, the end date will be set to 30 days from today.</small>
            </div>
...
<form method="post" action="add_video.php">
    <input type="hidden" name="return_to" value="create_groups.php">
